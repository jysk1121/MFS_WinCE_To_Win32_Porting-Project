...
`PROJECT_CONTEXT.md`는 저장소를 `Nexcom/` 단독으로 운영하던 시기(경로 `D:\...\Nexcom`, Reference를
git 미포함으로 기재)에 작성되어 **경로와 git 관련 설명은 현재와 다릅니다.** 작업 이력·미해결 과제는
여전히 유효하지만, 충돌하면 이 파일과 `Nexcom/CLAUDE.md`를 우선합니다.
