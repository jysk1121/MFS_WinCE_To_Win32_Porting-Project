...
class CA2GZIPT
{
  ...
  void Init(char *lpsz,int len=-1)
  {
	...
	// [#2053] NH KJW 2011.04.29
	//m_CurrentBufferSize=t_nBufferLength;
	//pgzip=m_buffer;
	m_CurrentBufferSize = NETBUF_RECV_SIZE - 1;
	pgzip = m_buffer = (LPGZIP)g_psz; // g_psz가 선언된 TranBizHost_Linq3.cpp에 GZipHelper.h가 include되어 있어, 별다른 extern 선언없이 사용가능함.
	// end of [#2053]
	...
	//m_outbuf = (Byte*)ALLOC(Z_BUFSIZE); // [#2053] NH KJW 2011.04.29 m_outbuf는 local배열로 수정함
 ...
 private:
   ...
   {
     if(buf==0) return 0;
	 if(Length+count>m_CurrentBufferSize) // [#2053] NH KJW 2011.04.29 m_CurrentBufferSize를 200K로 충분히 크게 잡아두었으므로, 새로 malloc()하는 루틴을 타지않을 것으로 예상.
	 {
 ...
 int destroy()
 {
    ...
    //TRYFREE(m_outbuf); // [#2053] NH KJW 2011.04.29
    return err;
 }
...
class CGZIP2AT
{
  ...
  void Init()
  {
	...
	//psz=m_buffer;
	m_CurrentBufferSize = NETBUF_RECV_SIZE - 1;
	psz = m_buffer = g_psz; // g_psz가 선언된 TranBizHost_Linq3.cpp에 GZipHelper.h가 include되어 있어, 별다른 extern 선언없이 사용가능함.
	// end of [#2053]
 ...
 int destroy()
 {
    ...
    if (m_z_err < 0) err = m_z_err;
    //TRYFREE(m_inbuf); // [#2053] NH KJW 2011.04.29
    return err;
}
