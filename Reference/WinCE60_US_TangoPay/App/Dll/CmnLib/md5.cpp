...
#include "stdafx.h"
//#include "MD5.h"
#include ".\Common\MD5.h"		// [#725] US KSK 2012.05.23
#include ".\Common\CmnLib.h"	// [#725] US KSK 2012.05.23
...
/// md5str, added by MK
///
int md5str ( TCHAR *str , unsigned long seed , MD5_CTX *mdContext , unsigned long length )
{
	...
	/// convert from TCHAR* to unsigned char*
	char *pstrDesc = new char[length+1];
    WideCharToMultiByte(CP_ACP , 0, str, -1, pstrDesc, length, NULL, NULL);

	MD5Update ( mdContext , (unsigned char *) pstrDesc , length ) ;
	delete []pstrDesc;
...
CString WINAPI GetMD5(CString strValue)
{
	MD5_CTX m_md5;
	CString tempHash;
	
	if (md5str(strValue.GetBuffer(0), 0, &m_md5, strValue.GetLength() ))
	{
		// convert to ASCII
		for (int i = 0; i < 16; i++)
		{
...
CString WINAPI GetSurchTableLicense(CString strValue)
{
	MD5_CTX m_md5;
	CString tempHash;
	
	CString sEncrypString;
	sEncrypString.Format(L"USING LOCAL SURCHARGE||TID=%s", strValue);
	
	if (md5str(sEncrypString.GetBuffer(0), 0, &m_md5, sEncrypString.GetLength() ))
	{
		int i;
