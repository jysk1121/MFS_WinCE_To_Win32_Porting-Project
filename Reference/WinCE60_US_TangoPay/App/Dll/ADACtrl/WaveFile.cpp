...
{
    RETAILMSG(0, (TEXT("ERROR: WaveFileError void")));
...
{
    RETAILMSG(0, (TEXT("ERROR: WaveFileError %d"), dwError));
    dwLastError = dwError;
    Close();
    return dwLastError;
...
{
    BOOL    bRtn = FALSE;		// [#2022] NH KSK 2011.02.22
    DWORD   dwRtn = 0;		// [#2022] NH KSK 2011.02.22
    BYTE    byTemp = 0;		// [#2022] NH KSK 2011.02.22
...
{
    BOOL    bRtn;
    DWORD   dwRtn = 0;	// [#2022] NH KSK 2011.02.21 Code Sonar 지적항목 대책

    // CHECK ME
    if (INVALID_HANDLE_VALUE == hFileSize)
    	{
              RETAILMSG(0, (TEXT("ERROR:WRITESIZE:  hFileSize == INVALID_HANDLE_VALUE")));
		return FALSE;
    	}
                         ...
                         LPVOID          *lplpInfo )
{
    ...
    if( INVALID_HANDLE_VALUE == hWaveData )
    {
        RETAILMSG(0, (TEXT("ERROR: CREATE:  hWaveData == INVALID_HANDLE_VALUE")));
    ...
    if( dwDesiredAccess & GENERIC_WRITE )
    {
        ...
        if( INVALID_HANDLE_VALUE == hRiffSize )
        {
            RETAILMSG(0, (TEXT("ERROR:CREATE:  hRiffSize == INVALID_HANDLE_VALUE")));
            return dwLastError = GetLastError();
        }
...
--*/
// [#2022] NH KSK 2011.02.21 Code Sonar 지적사항 대책으로 사용 안하는 함수이므로, 함수 주석 처리함
/*
                           ...
                           LPDWORD         lpNumberOfBytesWritten )
{
    ...
    if(NULL == lpWaveData) {
              RETAILMSG(0, (TEXT("ERROR: WRITEDATA:  NULL == lpWaveData")));
		dwLastError = ERROR_INVALID_PARAMETER;
		return bRtn = false;
    }
    ...
    {
        RETAILMSG(0, (TEXT("ERROR: WRITEDATA:  hWaveData == INVALID_HANDLE_VALUE")));
    }
    ...
    if( bRtn != 0 )
    {
        ...
        //     RETAILMSG(0, (TEXT("ERROR: Unable to set end of file %d"), GetLastError() ));
        ...
        if(hRiffSize == INVALID_HANDLE_VALUE)
            RETAILMSG(0, (TEXT("ERROR: WRITEDATA hRiffSize == INVALID_HANDLE_VALUE")));
        if(!WriteSize( hRiffSize, nRiffSize ))
             RETAILMSG(0, (TEXT("ERROR: Unable to write RIFF size to file")));
        if(!WriteSize( hWaveSize, nWaveSize ))
             RETAILMSG(0, (TEXT("ERROR: Unable to write WAVE size to file")));
    } // end if( bRtn )
    else
    {
        RETAILMSG(0, (TEXT("ERROR: In WriteData, bRtn was FALSE")));
        RETAILMSG(0, (TEXT("ERROR: WRITEDATA:  GetLastError Was %d"), dwLastError));
    }
                           ...
                           LPVOID*           lplpInfo )
{
	...
	if (hRiffSize != INVALID_HANDLE_VALUE)		// [#2022] NH KSK 2011.02.22
	{
		dwRtn = SetFilePointer( hRiffSize, 4, NULL, FILE_BEGIN );
		if( 0xFFFFFFFF == dwRtn ) return WaveFileError();
	}
