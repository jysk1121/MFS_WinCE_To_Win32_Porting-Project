...
#define 	ICEMV_APP_CNT			60				//VISA, MASTER, JCB, NonEMV etc..
...
typedef  struct
{
    ...
    unsigned char   IsrCntryCodeLen_Alpha2;  	// 5F55 Len								// [#2341] US Justin US COMMON AID
    unsigned char   IsrCntryCode_Alpha2[2];   	// 5F55 Value							// [#2341] US Justin US COMMON AID	
    unsigned char   IsrIdNo[3];         		// Issuer Identification Number(IIN)	// [#2341] US Justin US COMMON AID
...
// for Ver5.5
DLL_IMPORT int  emvkrnl_Sel_ApplicationSelection(unsigned char SupportPSE, EMVst_LIST_OF_AID_V5 *AIDList, int *CandidateCnt);
...
DLL_IMPORT int  emvkrnl_Sel_MakeDisplayAppList(unsigned char SupportUserConfirm, 
								   unsigned short CandidateCnt, 
								   EMV_ICC_APPS_INFORM_V5 *IccAppInform);
...
DLL_IMPORT void emvkrnl_Util_SetKernelLogType(unsigned char *ExtensionName, unsigned int ExtensionLen);
DLL_IMPORT void emvkrnl_Util_SetDebugFlag(int mode);
DLL_IMPORT void emvkrnl_Trans_Initial(void);

// Hyosung Tag Define
/* EMV TAG & CODE DEFINE*/
#define CRYPTOGRAM_INFO_DATA_TAG_9F27			_T("9F27")		// Max. 1 Byte	(transaction, reversal)
#define TERMINAL_VERIFICATION_RESULT_TAG_95		_T("95")		// Max. 5 Bytes	(transaction, reversal)
#define APP_CRYPTOGRAM_TAG_9F26					_T("9F26")		// Max. 8 Bytes	(transaction, reversal)
#define AMOUNT_AUTHORIZED_TAG_9F02				_T("9F02")		// Max. 6 Bytes	(transaction, reversal)
#define AMOUNT_OTHER_TAG_9F03					_T("9F03")		// Max. 6 Bytes	(transaction, reversal)
#define	APPLICATION_ID_TAG_9F06					_T("9F06")		// Max. 16 Bytes (transaction, reversal)
#define APP_VERSION_NUMBER_TAG_9F09				_T("9F09")		// Max. 2 Bytes	(transaction)
#define APP_INTERCHANGE_PROFILE_TAG_82			_T("82")		// Max. 2 Bytes	(transaction, reversal)
#define APP_TRANSACTION_COUNTER_TAG_9F36		_T("9F36")		// Max. 2 Bytes	(transaction, reversal)
#define DEDICATED_FILE_NAME_TAG_84				_T("84")		// Max. 16 Bytes	(transaction)
#define	TERMINAL_COUNTRY_CODE_TAG_9F1A			_T("9F1A")		// Max. 2 Bytes	(transaction, reversal)
#define TRANSACTION_CURRENCY_CODE_TAG_5F2A		_T("5F2A")		// Max. 2 Bytes	(transaction, reversal)
#define	TRANSACTION_DATE_TAG_9A					_T("9A")		// Max. 3 Bytes	(transaction, reversal)
#define TRANSACTION_TYPE_TAG_9C					_T("9C")		// Max. 1 Byte	(transaction, reversal)
#define RANDOM_NUMBER_TAG_9F37					_T("9F37")		// Max. 4 Bytes	(transaction, reversal)
#define ISSUER_APP_DATA_TAG_9F10				_T("9F10")		// Max. 32 Bytes (transaction, reversal)
#define IFD_SERIAL_NUMBER_TAG_9F1E				_T("9F1E")		// Max. 8 Bytes	(transaction, reversal)
#define	TERMINAL_CAPABILITIES_TAG_9F33			_T("9F33")		// Max. 3 Bytes	(transaction)
#define TERMINAL_TYPE_TAG_9F35					_T("9F35")		// Max. 1 Byte	(transaction)
#define CVM_RESULT_TAG_9F34						_T("9F34")		// Max. 3 Bytes	(transaction)
#define TRANSACTION_CATEGORY_CODE_TAG_9F53		_T("9F53")		// Max. 1 Byte	(transaction)	user defined tag(CA)
#define TRANSACTION_SEQUENCE_COUNTER_TAG_9F41	_T("9F41")		// Max. 4 Bytes	(transaction)
//#define ISSUER_SCRIPT_RESULTS_TAG_9F5B			_T("9F5B")		// Max. 5*n(n is received issuer script count)	(transaction, reversal) user defined tag(CA)
#define ISSUER_SCRIPT_RESULTS_TAG_DF05			_T("DF05")		// Max. 5*n(n is received issuer script count)	(transaction, reversal) user defined tag(CA)
#define APPLICATION_PAN_SEQ_TAG_5F34			_T("5F34")		// Max. 1 Byte	(transaction)
#define	CVM_LIST_TAG_8E							_T("8E")		// Max. 252 Bytes	(transaction)
#define POS_ENTRY_MODE_TAG_9F39					_T("9F39")		// Max. 1 Byte	(transaction)
#define TRANSACTION_STATUS_INFO_TAG_9B			_T("9B")		// Max. 2 Bytes	(transaction)
#define	AUTHORISATION_RESPONSE_TAG_8A			_T("8A")		// Max. 2 Bytes (response)
#define ISSUER_AUTHENTICATION_DATA_TAG_91		_T("91")		// Max. 16 Bytes (response)
//#define ISSUER_SCRIPT1_TAG_71					_T("71")		// Var.			(response)
//#define ISSUER_SCRIPT2_TAG_72					_T("72")		// Var.			(response)
#define ISSUER_SCRIPT_ID_TAG_9F18				_T("9F18")		// Max. 4 Bytes(transaction, reversal)
#define TRACK2_EQUIVALENT_DATA_TAG_57			_T("57")
#define APP_EXPIRED_DATE_TAG_5F24				_T("5F24")
#define APPLICATION_PAN_TAG_5A					_T("5A")
#define TRANSACTION_TIME_TAG_9F21				_T("9F21")
#define ADD_TERMINAL_CAPABILITIES_TAG_9F40		_T("9F40")
#define RESPONSE_CODE_TAG_8A					_T("8A")		// Max. 2 Bytes	(transaction)	// [#2188] NH KSK 2013.05.27

// [#2115] MX KSK 2012.02.04
#define JNL_IAD									_T("9100")
#define JNL_TVR									_T("9500")
#define APPLICATION_ID							_T("4F00")
#define APPLICATION_LABEL						_T("5000")
#define SERVICE_CODE							_T("5F30")
#define ISSUER_ACTION_CODE						_T("9F0E")
// end of [#2115]

// [#2353] AU KSK 2015.06.28
#define	JNL_AIP_8200							_T("8200")
#define JNL_ARC_8A00							_T("8A00")
#define	JNL_TSI_9B00							_T("9B00")
#define JNL_TRANSTYPE_9C00						_T("9C00")
// end of [#2353]
