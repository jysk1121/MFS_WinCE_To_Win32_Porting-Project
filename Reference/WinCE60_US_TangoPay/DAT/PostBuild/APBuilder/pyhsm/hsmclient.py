...
class HsmClient:
    ...
    def __init_libhsm(self):
        ...
        # connect() was renamed to HSM_connect to avoid a name-clash with the C library function
        self.__libhsm.HSM_connect.argtypes = [c_char_p, c_ulong, c_char_p, c_ulong]
    ...
    def __connect(self, p11_path):
        ...
        # This function name has to be renamed because the underlying c-library cannot overload the
        # connect() function name. It has been renamed to HSM_connect
        rv = self.__libhsm.HSM_connect(msg, len(msg), p11_path_bytes, len(p11_path_bytes))
        if rv == 0:
            raise HsmError(bytes_to_str(msg.value))
