...
#include "./INC/bufferfill.h"
#include <pnp.h>
#include <winbase.h>
#include <tchar.h>
#include "./INC/clparse.h"
...
#define SUPPORT_MJPG	0

// global media types to use as the defaults.
AM_MEDIA_TYPE g_mt;
GSM610WAVEFORMAT g_pwfx;
int gBurstCount = 0;
static INT16 gPictureNumber = -1;
extern UINT8 g_CaptureResolutionIndex;

//TCHAR msgBuff[MAX_PATH];
// 17.10.25 not use
//HRESULT addSampleGrabber(IGraphBuilder *pGraph, IBaseFilter *pSourceFil, IBaseFilter **pGrab);

// calback function when samplegrabber filter f=get media sample
void OnSampleProcessed(BYTE* pData, long len, long m_Height, long m_Width, long m_Stride)
{
	static DWORD lastTime;
	//NKDbgPrintfW(L"pData %#08x %lu \n",pData,GetTickCount()-lastTime);
	lastTime = GetTickCount();
}

//ISampleGrabber *m_pGrab = NULL;
...
CCaptureFramework::ParseCommandLine(LPCTSTR tszCommandLine)
{
    ...
    else if(cCLPparser.GetOpt(TEXT("?")))
    {
        ...
        //Log((TEXT("        /OutputKeyEvents - enables the test to output //Debug(ging information for missing events, //SKIP( outputting high volume events."));
        //Log((TEXT("        /OutputAllEvents - enables the test to output //Debug(ging information for missing events, outputs every event."));
        //Log((TEXT("        /OutputFilterInformation - enables the test to output information about the graph topo//Log(y."));
    ...
    if(tszFileName)
    {
		...
		if(m_lCaptureNumber >= -1)
		{
			...
			do
			{
				int nStringLength = MAX_PATH;
				WIN32_FIND_DATA fd;
				GetVideoCaptureFileName(tszPictureName, &nStringLength);
				hFile = FindFirstFile(tszPictureName, &fd);
				if(INVALID_HANDLE_VALUE != hFile)
				{
					CloseHandle(hFile);	
					//Increament and find file
					m_lCaptureNumber++;
				}	
			}while(hFile != INVALID_HANDLE_VALUE);
...
CCaptureFramework::PropogateFilenameToCaptureFilter()
{
	...
	//FAIL(TEXT("Video save %s"),tszPictureName); 
    if(m_pVideoFileSink)
    {
...
//add custome sample grabber to the graph
// 17.10.25 not use
//HRESULT CCaptureFramework::addSampleGrabber(IGraphBuilder *pGraph, IBaseFilter *pSourceFil, IBaseFilter **pGrab)
//{
//	HRESULT hr = S_OK;
//	IBaseFilter *pSampleGrab = new CSampleGrabber(NULL, &hr, TRUE);
//	if(FAILED(hr))
//	{
//		NKDbgPrintfW(L"ERROR- CSampleGrabber %d\r\n", GetLastError());
//		return hr;
//	}
//	
//	NKDbgPrintfW(L"CSampleGrabber obj\n");
//
//	pSampleGrab->AddRef();
//	hr = pGraph->AddFilter(pSampleGrab, L"samplegrabber");
//	if(FAILED(hr))
//	{
//		NKDbgPrintfW(L"ERROR- AddFilter CSampleGrabber %d\r\n", GetLastError());
//		return hr;
//	}
//	
//	// insert it after the vcap filter
//    /* hr = InsertFilter(pSourceFil, PINDIR_INPUT, NULL, NULL, 0, pSampleGrab);
//    if(FAILED(hr))
//    {
...
//    } */
//
//	pSampleGrab->QueryInterface(IID_ISampleGrabber, (void**)&m_pGrab);
//	if(m_pGrab)
//	{
//		m_pGrab->RegisterCallback(OnSampleProcessed);
...
CCaptureFramework::Init(HWND OwnerWnd, RECT *rcCoordinates, DWORD dwComponentsOptions)
{
	...
	if(SUPPORT_MJPG)
	{
		dwComponentsOptions &= ~VIDEO_ENCODER;
	}
...
#if (_WIN32_WCE == 0x600)
	HKEY hkeyFindTeg = NULL;
    DWORD dwDataType;
    TCHAR tszPrefix[MAX_DEVCLASS_NAMELEN];
    DWORD dwDataSize = sizeof(tszPrefix);
    DWORD dwIndex;
    DWORD dwIndexSize = sizeof(DWORD);
	TCHAR search[] = L"Nvidia";
	DWORD TegProc;
	int i;

	//[HKEY_CLASSES_ROOT\CLSID\{CDC2B6EC-87C0-489d-BE56-F522D8406C4B}]
	//Fix for Finding Windows CE 600 Tegra and Renering on GDI Mode
    if (RegOpenKeyEx(HKEY_CLASSES_ROOT,
                            TEXT("CLSID\\{CDC2B6EC-87C0-489d-BE56-F522D8406C4B}"), 
                            0, // Reseved. Must == 0.
                            0, // Must == 0.
                            &hkeyFindTeg) == ERROR_SUCCESS)
    {
        if(RegQueryValueEx(hkeyFindTeg,
                                TEXT("FriendlyName"),
                                NULL,
                                &dwDataType,
                                (LPBYTE) tszPrefix,
        ...
        {
			RETAILMSG(1, ( TEXT("Find %s"),tszPrefix));						

			for(i=0; i<wcslen(search); i++)
				{
					if(search[i] == tszPrefix[i])
					{
						continue;
					}
					else
				...
				if( i == wcslen(search))
				{
					SwitchVideoRenderMode();				
				}
        }
        RegCloseKey(hkeyFindTeg);
    }
#endif
	
	/* IVMRFilterConfig    *pVMRConfig;
 
	hr = m_pVideoRenderer.QueryInterface(&pVMRConfig );
	...
	DWORD dwRenderFlags;
	DWORD Mode;
	pVMRConfig->GetRenderingPrefs(&dwRenderFlags);
	//FAIL(TEXT("%d"),dwRenderFlags);
	
	pVMRConfig->GetRenderingMode(&Mode); 
	//FAIL(TEXT("%d"),Mode);
	
	//pVMRConfig->SetRenderingMode(AM_VIDEO_RENDERER_MODE_DDRAW);
...
CCaptureFramework::CoCreateComponents()
{
    ...
    {
      #if (_WIN32_WCE == 0x600)  
		hr = CoCreateInstance( CLSID_VideoRenderer, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**) &m_pVideoRenderer );//VideoRenderer
	  #else	
		hr = CoCreateInstance( CLSID_VideoMixingRenderer, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**) &m_pVideoRenderer );//VideoRenderer
	 #endif	   
    ...
    if(m_bForceColorConversion)
    {
        ...
        {
			 //DETAIL((TEXT("Creating the color converter Called."));           
			hr = CoCreateInstance( CLSID_Colour, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**) &m_pColorConverter );
            if(FAILED(hr) || m_pColorConverter == NULL)
            {
                //DETAIL((TEXT("Creating the color converter failed."));
	...
	//hr = GetStreamCaps(STREAM_CAPTURE,g_CaptureResolutionIndex, &pmtConfig, (BYTE*)&scc);
	////DETAIL((TEXT("pmtConfig->subtype 0x%X"),pmtConfig->subtype);
...
CCaptureFramework::AddInterfaces()
{
    ...
    if(m_pVideoRenderer)
    {
		...
		{
			m_pVideoRendererBasicVideo->SetDestinationPosition(0,0,160,120);
		}
	#else
		hr = m_pGraph.QueryInterface( &m_pVideoWindow );
        if(FAILED(hr) || m_pVideoWindow == NULL)
        {
            //FAIL(TEXT("CCaptureFramework: Retrieving the IVideoWindow failed."));
            goto cleanup;
		...
		/*hr = m_pVideoWindow.QueryInterface( &m_pVideoRendererMode );
        if(FAILED(hr) || m_pVideoRendererMode == NULL)
        {
            //FAIL(TEXT("CCaptureFramework: Unable to retrieve the video renderer mode."));
            goto cleanup;
        ...
        hr = m_pGraph.QueryInterface( &m_pVideoRendererBasicVideo );
        if(FAILED(hr) || m_pVideoRendererBasicVideo == NULL)
        {
            //FAIL(TEXT("CCaptureFramework: Unable to retrieve the video renderer BasicVideo interface."));
            goto cleanup;
...
void CCaptureFramework:: InitialisePreview()
{
	m_pVideoPropertyBag = NULL;
	m_pVideoCapture.QueryInterface( &m_pVideoPropertyBag );
	m_pVideoPropertyBag->Load( &PropBag, NULL );
}
...
CCaptureFramework::InitFilters()
{
	...
	{
	}	//FAIL(TEXT("CCaptureFramework: m_nSelectedCameraDevice is not valid\r\n"));
    ...
    if(m_pVideoEncoderDMO)
    {
		...
		//FAIL(TEXT("CCaptureFramework: m_pVideoEncoderDMO is null\r\n"));
    ...
    if(m_pAudioEncoderDMO)
    {
		...
		//else
		////FAIL(TEXT("CCaptureFramework: Loading the video property bag data succsess."));
    }
	else
	{
	}	//FAIL(TEXT("CCaptureFramework: m_pAudioEncoderDMO is null\r\n"));
	...
	{
		//FAIL(TEXT("CCaptureFramework: m_pStillSink is null\r\n"));
	}
...
CCaptureFramework::InitializeStreamFormats()
{
	...
	//m_pamtStillStream->subtype = MEDIASUBTYPE_UYVY;//added  
	//m_pamtPreviewStream->subtype = MEDIASUBTYPE_RGB565;//added 
	...
	/*if(FAILED(hr = m_pPreviewStreamConfig->GetFormat(&m_pamtPreviewStream)))
    {
        //FAIL(TEXT("CCaptureFramework: Unable to initialize the preview format."));
        goto cleanup;
    }*/
	/*else 
	{
		RETAILMSG(1,(TEXT("preview and still format 0x%x 0x%x \r\n"),m_pamtPreviewStream->subtype,m_pamtStillStream->subtype));
	}*/
...
CCaptureFramework::RenderGraph()
{
	...
	if(!bOnlyOnce)
	{
		m_pCaptureGraphBuilder->FindInterface(
		...
		if(iSize == sizeof(CS_VIDEO_STREAM_CONFIG_CAPS))
		{
			for(int iFormat = 0; iFormat < iCount; ++iFormat)
			{	
				CS_VIDEO_STREAM_CONFIG_CAPS scc;
				if(SUCCEEDED(pConfig->GetStreamCaps(iFormat, &pmtConfig, (BYTE*)&scc)))
				{
					CS_VIDEOINFOHEADER *pVih = (CS_VIDEOINFOHEADER*)pmtConfig->pbFormat;
					
					RETAILMSG(1, (TEXT("VWid:%d Vhei:%d 0x%X\n"),pVih->bmiHeader.biWidth,pVih->bmiHeader.biHeight,pmtConfig->subtype.Data1));
	
					//MEDIASUBTYPE_MJPG MEDIASUBTYPE_YUY2
					if(pVih->bmiHeader.biWidth == 320 && pVih->bmiHeader.biHeight == 240 && pmtConfig->subtype == MEDIASUBTYPE_YUY2)
					{
						pConfig->SetFormat(pmtConfig);
						RETAILMSG(1, (TEXT("SetFormat\n")));
						break;
					}
    ...
    if(m_pVideoCapture && m_pVideoRenderer)
    {
		...
		{
			// 17.10.25 not use
			//hr = addSampleGrabber(m_pGraph, m_pVideoRenderer, &pGrab);
	        // Render the streams
	        hr = m_pCaptureGraphBuilder->RenderStream( &PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, m_pVideoCapture,pGrab , m_pVideoRenderer );//m_pColorConverter
	        if(FAILED(hr))
	        {
	            //FAIL(TEXT("CCaptureFramework: Rendering the preview failed. 0x%x"),hr);
	            goto cleanup;
	        }
		}
		else
		{
			  // Render the streams
	        hr = m_pCaptureGraphBuilder->RenderStream( &PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, m_pVideoCapture,NULL , m_pVideoRenderer );//m_pColorConverter
	        if(FAILED(hr))
	        {
	            //FAIL(TEXT("CCaptureFramework: Rendering the preview failed. 0x%x"),hr);
	            goto cleanup;
    ...
    if(m_pVideoWindow)
	{
		 //hr = m_pVideoWindow->put_BackgroundPalette(OATRUE);
         //expected to fail if there's no VCF
        if(FAILED(hr) || !m_pVideoCapture)
        {
            //DETAIL((TEXT("CCaptureFramework: background pallate failed."));
        }
		else
        {
            ////DETAIL((TEXT("CCaptureFramework: background pallate succsess."));
        }
        ...
        //{
          //  hr = m_pVideoWindow->SetWindowPosition(0, m_rcLocation.top, m_lDefaultWidth, m_lDefaultHeight);
			hr = m_pVideoWindow->SetWindowPosition(m_rcLocation.left, m_rcLocation.top, m_rcLocation.right, m_rcLocation.bottom);
			if(FAILED(hr))
            {
	...
	if(m_bInsertPreviewDiagnostics || m_bInsertStillDiagnostics || m_bInsertCaptureDiagnostics)
    {
        ...
        if(FAILED(hr) || pGrabber == NULL)
        {
            ...
            if(S_OK != RegisterDll(TEXT("CameraGrabber.dll")))
                if(S_OK != RegisterDll(TEXT("\\USB\\CameraGrabber.dll")))
...
HRESULT
CCaptureFramework::SetFocusGrid()
{
	HRESULT hr = S_OK;
...
HRESULT
CCaptureFramework::RemoveFocusGrid()
{
	HRESULT hr = S_OK;
...
CCaptureFramework::RunGraph()
{
	...
	// if(i==1)
	 //{
	 //m_pVideoWindow->put_BackgroundPalette(OATRUE);
	// //FAIL(TEXT("CCaptureFramework:i=0\r\n"));
	// i=0;
	// }
	 //else
	// {
	// m_pVideoWindow->SetWindowForeground(OATRUE);
	// //FAIL(TEXT("CCaptureFramework:i=1\r\n"));
	// i=1;
	// }
	// m_pVideoWindow->put_WindowState(SW_SHOW);
	 
	 //m_pVideoWindow->put_BackgroundPalette(OAFALSE);//added
	...
	{
		//DisplayPinsOfFilter(m_pVideoCapture);
	}
...
CCaptureFramework::Set(int nProperty, long lProperty, long lValue, long lFlags)
{
    ...
    switch(nProperty)
    {
        case PROPERTY_CAMERACONTROL:
			...
			{
				//DETAIL((TEXT("CCaptureFramework: Failed to set the PROPERTY_CAMERACONTROL value. hr = %lu"),hr);
				//DETAIL((TEXT("CCaptureFramework: Failed to set the PROPERTY_CAMERACONTROL value. GetLastERROR  = %lu"),GetLastERROR());
            
			}break;
        case PROPERTY_VIDPROCAMP:
            if(FAILED(hr = m_pVideoProcAmp->Set(lProperty, lValue, lFlags)))
			{            
				//DETAIL((TEXT("CCaptureFramework: Failed to set the PROPERTY_VIDPROCAMP value. hr = %lu"),hr);
				//DETAIL((TEXT("CCaptureFramework: Failed to set the PROPERTY_VIDPROCAMP value. GetLastERROR  = %lu"),GetLastERROR());
			}
            break;
        default:
            hr = E_FAIL;
...
CCaptureFramework::GetStreamCaps(int nStream, int nIndex, AM_MEDIA_TYPE **pFormat, BYTE *pSCC)
{
    ...
    switch(nStream)
    {
        case STREAM_PREVIEW:
			...
			{
                ////DETAIL((TEXT("CCaptureFramework: succsess to get the preview stream caps."));           
			}
		   break;
        case STREAM_STILL:
            // if the component isn't available, fail the GetStreamCaps call.
			...
			{
				////DETAIL((TEXT("CCaptureFramework: succsess to get the still stream caps."));
			}
			break;
        case STREAM_CAPTURE:
            // if the component isn't available, fail the GetStreamCaps call.
			...
			{
				////DETAIL((TEXT("CCaptureFramework: success to get the capture stream caps."));
			}
			break;
        case STREAM_AUDIO:
            // if the component isn't available, fail the GetStreamCaps call.
			...
			{
			   // //DETAIL((TEXT("CCaptureFramework: success to get the audio stream caps."));
			}
			break;
        default:
...
CCaptureFramework::SetFormat(int nStream, AM_MEDIA_TYPE *pFormat)
{
    ...
    *ppSetFormat = NULL;
    ...
    // if a valid format was //PASS(ed in, grab a copy of it
    if(pFormat)
    {
...
CCaptureFramework::GetFormat(int nStream, AM_MEDIA_TYPE **pFormat)
{
    ...
    switch(nStream)
    {
        case STREAM_PREVIEW:
			...
			{
				//FAIL(TEXT("CCaptureFramework: succedd to retrieve the preview format."));
			}
            break;
        case STREAM_STILL:
            // if the component isn't available, fail the GetFormat call.
			...
			{
				//FAIL(TEXT("CCaptureFramework: succedd to retrieve the still format."));
			}
            break;
        case STREAM_CAPTURE:
            // if the component isn't available, fail the GetFormat call.
			...
			{
				////FAIL(TEXT("CCaptureFramework: succedd to retrieve the capture format."));
			}
            break;
        case STREAM_AUDIO:
            // if the component isn't available, fail the GetFormat call.
			...
			{
				////FAIL(TEXT("CCaptureFramework: succedd to retrieve the audio format."));
			}
			break;
        default:
            //FAIL(TEXT("CCaptureFramework: Invalid stream requested."));
...
CCaptureFramework::CaptureStillImage()
{
	...
	/* if(m_lPictureNumber >= -1)
    {
        ...
        do
        {
            ...
            int nStringLength = MAX_PATH;
            WIN32_FIND_DATA fd;
			////Log((TEXT("%u m_lPictureNumber gPictureNumber %u "), m_lPictureNumber , gPictureNumber);
			RETAILMSG(//Debug(_ENABLE,(TEXT("m_lPictureNumber %u gPictureNumber %u \r\n"), m_lPictureNumber , gPictureNumber));
            m_lPictureNumber = gPictureNumber  ;
            m_lPictureNumber++;
			gPictureNumber = m_lPictureNumber; 
			RETAILMSG(//Debug(_ENABLE,(TEXT("m_lPictureNumber1 %u gPictureNumber1 %u \r\n"), m_lPictureNumber , gPictureNumber));
            GetStillCaptureFileName(tszPictureName, &nStringLength);
            hFile = FindFirstFile(tszPictureName, &fd);
	...
	hr = m_EventSink.WaitOnEvent(&dse, MEDIAEVENT_TIMEOUT);	// Still Capture fail시 timeout 조정 10sec -> 5sec
    if(FAILED(hr))
    {
...
CCaptureFramework::CaptureStillImageBurst(int nBurstCount)
{
    ...
    RETAILMSG(1,(TEXT("  burst capture hr = 0x%x \r\n"),hr));
	if(FAILED(hr))
    {
        //FAIL(TEXT("CCaptureFramework: Failed to retrieve the event indicating the capture completed."));
        OutputMissingEventsInformation(EC_CAP_FILE_COMPLETED);
...
CCaptureFramework::StartStreamCapture()
{
    ...
    DShowEvent dse[2];
	//VIDEORENDERER_STATS statData;
	//GetVideoRenderStats(&statData);
...
CCaptureFramework::VerifyVideoFileCaptured(TCHAR *tszVideoFileName)
{
	...
	//FAIL(TEXT("VerifyVideoFileCaptured"));
	
	//increment the file name before the capture 
	m_lCaptureNumber++;
    ...
    if(m_pfnVerifyVideoFile)
    {
		...
		{
			//FAIL(TEXT("Unable to verify, no file name available"));
		}
    }
...
CCaptureFramework::VerifyPreviewWindow()
{
    ...
    {
		// Camera Mode가 ForceManual일 경우 error return 처리
		/*
        int nMessageBoxResult;

        nMessageBoxResult = MessageBox(m_hwndOwner, TEXT("Is the preview image on the screen correct?"), 
                                                                TEXT("DShowTestApp"), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON1 | MB_SETFOREGROUND);
...
CCaptureFramework::VerifyStillImageLocation(TCHAR *tszImageName, int nTickCountBefore, int nTickCountAfter)
{
    ...
    else
    {
        ...
        nMessageBoxResult = MessageBox(NULL, TEXT("Is the image on the screen the image expected to be captured?"), 
                                                                TEXT("DShowTestApp"), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON1 );
        if(IDNO == nMessageBoxResult)
        {
...
CCaptureFramework::GetVideoRenderStats(VIDEORENDERER_STATS *statData)
{
	...
	/*//FAIL(TEXT("CCaptureFramework: AvgTimeperFrame. 0x%x"),AvgTimeperFrame);
	//FAIL(TEXT("CCaptureFramework: statData->dFrameRate. 0x%f"),statData->dFrameRate);
    //FAIL(TEXT("CCaptureFramework: statData->lSourceWidth. 0x%x"),statData->lSourceWidth);
	//FAIL(TEXT("CCaptureFramework: statData->lSourceHeight. 0x%x"),statData->lSourceHeight);
	//FAIL(TEXT("CCaptureFramework: statData->lDestWidth. 0x%x"),statData->lDestWidth);
	//FAIL(TEXT("CCaptureFramework: statData->lDestHeight. 0x%x"),statData->lDestHeight);
	*/
    // no GUID associated in atlbase.h
   // hr = m_pVideoRendererBasicVideo->QueryInterface(IID_IQualProp, (void**) &pQP);
	hr = m_pVideoRenderer->QueryInterface(IID_IQualProp, (void**) &pQP);
    if(FAILED(hr))
    {
	...
	//RETAILMSG(1,(TEXT("CCaptureFramework: statData->dActualRate %d \r\n"),(UINT32)statData->dActualRate));
    //RETAILMSG(1,(TEXT("CCaptureFramework: statData->lFramesDropped. %d \r\n"),(UINT32)statData->lFramesDropped));
	//wsprintf(msgBuff,L"Frame Rate %d Frame Dropped %d",(UINT32)statData->dActualRate,(UINT32)statData->lFramesDropped);
	//MessageBox(m_hwndOwner,msgBuff,L"DShowTestApp",MB_ICONINFORMATION);
	/*//FAIL(TEXT("CCaptureFramework: statData->dActualRate. 0x%f"),statData->dActualRate);
    //FAIL(TEXT("CCaptureFramework: statData->lFramesDropped. 0x%x"),statData->lFramesDropped);*/
cleanup:
    return hr;
}
...
CCaptureFramework::VerifyDriverImage(HBITMAP hbmp, int nStartTime, int nStopTime, DWORD dwOrientation)
{
    ...
    if(NULL == hbmp)
    {
        //FAIL(TEXT("CCaptureFramework: Invalid HBITMAP //PASS(ed."));
        hr = E_FAIL;
        goto cleanup;
    }
    ...
    {
        //FAIL(TEXT("CCaptureFramework: HBitmap //PASS(ed not a DIBSection."));
        hr = E_FAIL;
        goto cleanup;
    }
...
CCaptureFramework::OutputEventInformation(LONG lEventCode, LONG lParam1, LONG lParam2)
{
	switch(lEventCode)
    {
        ...
        case EC_FILE_CLOSED:
            ...
            break;
        case EC_ERROR//ABORT(EX:
            //Log((TEXT("Received an ERROR//ABORT(ex event, 0x%08x, 0x%08x."), lParam1, lParam2);
            break;
        case EC_EOS_SOON:
...
CCaptureFramework::DisplayPinsOfFilter(IBaseFilter *pFilter)
{
    ...
    if (SUCCEEDED(hr))
    {
	   ...
	   {
		//	RETAILMSG(1,(TEXT("0 "))); 
		//	if( pPin->EnumMediaTypes(&pEnumMediaType)==S_OK)
		//	 {
		//		 RETAILMSG(1,(TEXT("1")));
		//		if(pEnumMediaType->Next(1,&m_pstreamformat,NULL)== S_OK)
		//		{
		//			RETAILMSG(1,(TEXT("2 ")));
		//			RETAILMSG(1,(TEXT("m_pamtPreviewStream subtype 0x%x \r\n"),m_pstreamformat->subtype));
		//		}
		//	 }

		//	if(pEnum->Next(1, &pPin,NULL) == S_OK)
		//	{
		//		RETAILMSG(1,(TEXT("0 "))); 
		//		if( pPin->EnumMediaTypes(&pEnumMediaType)==S_OK)
		//		{
...
CCaptureFramework::InsertFilter(IBaseFilter *pFilterBase, PIN_DIRECTION pindir, const GUID *pCategory, const GUID *pType, int index, IBaseFilter *pFilterToInsert)
{
    ...
    if(pindir == PINDIR_INPUT)
    {
		...
		{
			//FAIL(TEXT("CCaptureFramework::InsertFilter: Finding the requested pin failed."));        
		}
    }
    else
    {
    ...
    if(SUCCEEDED(hr))
    {
        ...
        if(SUCCEEDED(hr))
        {
			...
			{
				//FAIL(TEXT("CCaptureFramework::InsertFilter: Failed to enumerate the intermediate input pins."));
			}
            ...
            if(SUCCEEDED(hr))
            {
				...
				{//FAIL(TEXT("CCaptureFramework::InsertFilter: Failed to enumerate the intermediate output pins."));
				}
                ...
                if(SUCCEEDED(hr))
                {
                    if(SUCCEEDED(hr = m_pGraph->Disconnect(pPinUpstreamOut)))
                    {
                        if(SUCCEEDED(hr = m_pGraph->Disconnect(pPinDownstreamIn)))
                        {
							...
							{
								//FAIL(TEXT("CCaptureFramework::InsertFilter: Failed to connect the upstream output pin to the intermediate input."));
							}

                        }
                        else 
						{
							//FAIL(TEXT("CCaptureFramework::InsertFilter: Failed to disconnect the downstream input pin for insertion."));
						}
                    }
                    else 
					{
						//FAIL(TEXT("CCaptureFramework::InsertFilter: Failed to disconnect the upstream output pin for insertion."));
					}
                }
            }
        }
		...
		{
			//FAIL(TEXT("CCaptureFramework::InsertFilter: Failed retrieving the connection media type."));
			}
    }
...
HRESULT CCaptureFramework::PlayFile()
{
	HRESULT hr = S_OK;
    hr = m_pGraph->RenderFile(L"\\test0.asf", NULL);
	if (SUCCEEDED(hr))
    {
        // Run the graph.
        hr = m_pMediaControl->Run();
...
BOOL CCaptureFramework::IsExistCameraDriver()
{
	HRESULT hr = S_OK;
	BOOL bRet = FALSE;
	...
	{
		GetDriverList(&ptszCameraDrivers, &nDriverCount);
