...
#include "../../EagleCE_Framework/EagleCE_Framework/EagleLogger.h"
	...
	#include <wingdi.h>
	#include <windows.h>
	// stdafx.h defines VC_EXTRALEAN, so <windows.h> does not pull in the
	// multimedia API that PlaySound()/SND_ASYNC live in.
	#include <mmsystem.h>
...
/** *************************************************************
*	@brief	CRequest 생성자 
*	@param	REQ_TYPE type 요청에 대한 Type(REQ_KEYEVENT,REQ_SETFOCUS,REQ_FSCMD 등)
*	@param	CString Name
*	@param	CString Data
*****************************************************************/
...
/** *************************************************************
*	@brief	CRequest 생성자
*	@param	CRequest &rRequest Request객체
*****************************************************************/
CRequest::CRequest(const CRequest &rRequest)
{
	...
	m_rcUpdate = rRequest.m_rcUpdate;
	m_Point = rRequest.m_Point;
}

/** *************************************************************
*	@brief	Request에 대한 Type Read
*	@param	없음
*	@retval	REQ_TYPE
*****************************************************************/
...
CNHLinkedList<CRequest>	CEventHandler::m_Requests;
...
{
	return m_Requests.PutOnTailOfList(_T(""), rRequest);
}
...
/** *************************************************************
*	@brief	Event(Command)별 처리 수행
*	@param	CRequest &Request	설정된 Command 구조체
*****************************************************************/
BOOL CEventHandler::Handler(CRequest &Request)
{
	...
	case REQ_SETBLINKMODE:
		SetAPBlinkMode(CUtil::StringToInt(Request.GetData()));
		break;
...
RECT CNControl::GetAdjustScreenRate(int left, int top, int width, int height)
{
...
// 		m_fScreenRateY = (float)((float)currentY / (float)BASIC_SCREEN_Y);
		m_fScreenRateX = (float)(1);
		m_fScreenRateY = (float)(1);
		///////////////////////////////////////////////////////////////////////

		// 에러 발생시 무조건1:1 배율로 화면 조정함
		if (m_fScreenRateX == 0 || m_fScreenRateY == 0)
		{
			m_fScreenRateX = 1;
			m_fScreenRateY = 1;
...
{
	m_rcDisplay = GetAdjustScreenRate(left, top, width, height);
}
...
/** *********************************************************
*	@brief		control name 설정
*	@param	LPCTSTR str1	APStateXX 설정
*	@param	LPCTSTR str2	APValueXX 설정
*	@retval		없음
************************************************************/
...
CNTextBox::CNTextBox(CTL_TYPE type) : CNShape(type)
{
	...
	//Add vertical allignement
	m_VertAlign = CENTER_V;
...
/** *************************************************************
*	@brief		Blink 수행 여부를 설정
*	@details	없음.
*	@param	BOOL BlinkOn	Blink 수행 여부
*	@retval		없음
*****************************************************************/
...
*	@retval		없음
*****************************************************************/
void CNTextBox::SetBlinkOption(BOOL BlinkOn)
{
	m_bBlinkOption = BlinkOn;
}
...
//Add vertical alligment
void CNTextBox::SetTextVerticalAlign(TEXTVERTALIGN Align)
{
	m_VertAlign = Align;
}
	...
	if (strDrawText.GetLength())
	{
		...
		if (!m_bBlinking)
		{
			...
			if (strDrawText.GetLength() > 0)
			{
				...
				{
					if((rcClient.bottom - calcHeight) > 0)
						rcClient.top = (int)(rcClient.bottom - calcHeight);
				}
...
{
	if ((bBlinkOn == TRUE) && (m_bBlinkOption == TRUE))
	{
...
/** *************************************************************
*	@brief EditBox 속성(최대 글자수, TextID 등) 및 문자열 설정
*	@param	CString &strName	destination name 
*	@param	CString &strData	설정할 data
*	@retval	없음
*****************************************************************/
...
//	if (m_bFocus)
	if ( (TRUE == m_bFocus) && (FALSE == m_bKeyDisable) )
	{
		...
		// numeric key.
		else if ((strKey.GetLength() == 1) || (m_TextType == TTYPE_IPADDRESS && strKey == DES_STAR))
		{
			if (m_TextType == TTYPE_IPADDRESS && strKey == DES_STAR)
				strKey = _T(".");
...
/** *************************************************************
*	@brief	입력받을 최대 글자수 설정
*	@param	int nMaxChar	최대 글자수 
*	@retval	없음
*****************************************************************/
...
/** *************************************************************
*	@brief	입력받을 최소 글자수 설정
*	@param	int nMaxChar	최소 글자수 
*	@retval	없음
*****************************************************************/
...
/** *************************************************************
*	@brief	Key정보 입력되지 않도록 설정
*	@param	CString &strName	dest name
*	@retval	BOOL FALSE
*****************************************************************/
...
/** *************************************************************
*	@brief	Table의 row, column 설정
*	@param	int nRow	:	row
*	@param	int nColumn	:	column
*	@retval	없음
*****************************************************************/
...
/** *************************************************************
*	@brief	Guide font 설정
*	@param	CNFont *pNormal :	normal font
*	@param	CNFont *pSelect :	select font
*	@retval	없음
*****************************************************************/
...
/** *************************************************************
*	@brief	Guide TextColor 설정
*	@param	CNColor *pNormal :	normal color
*	@param	CNColor *pSelect :	select color
*	@retval	없음
*****************************************************************/
...
/** *************************************************************
*	@brief	Guide Brush 설정
*	@param	CNBrush *pNormal :	normal brush
*	@param	CNBrush *pSelect :	select brush
*	@retval	없음
*****************************************************************/
...
/** *************************************************************
*	@brief	Element Pen 설정
*	@param	CNPen *pDisable :	disable pen
*	@param	CNPen *pEnable :	enable pen
*	@param	CNPen *pSelect :	select pen
*	@retval	없음
*****************************************************************/
...
/** *************************************************************
*	@brief	Element Font 설정
*	@param	CNFont *pDisable :	disable font
*	@param	CNFont *pEnable :	enable font
*	@param	CNFont *pSelect :	select font
*	@retval	없음
*****************************************************************/
...
/** *************************************************************
*	@brief	Element Brush 설정
*	@param	CNBrush *pDisable :	disable brush
*	@param	CNBrush *pEnable :	enable brush
*	@param	CNBrush *pSelect :	select brush
*	@retval	없음
*****************************************************************/
...
/** *************************************************************
*	@brief	Element TextColor 설정
*	@param	CNColor *pDisable :	disable text color
*	@param	CNColor *pEnable :	enable text color
*	@param	CNColor *pSelect :	select text color
*	@retval	없음
*****************************************************************/
...
BOOL CNTableEditTextBox::Initialize(void)
{
	...
	//	nElementWidth = tm.tmAveCharWidth + (tm.tmExternalLeading * 5);
		nElementWidth = (int)((tm.tmAveCharWidth*1.8) + (tm.tmExternalLeading * 5)); //*LEH
		nElementHeight = (int)(tm.tmHeight*1.7 + (tm.tmExternalLeading * 4));
		...
		for (col = 0; col < MAX_TABLE_COL; col++)
		{
			...
			else
			{
				if (col == 0)
					...
					left = nStartX-10 + (col * nElementWidth);
...
/** *************************************************************
*	@brief	Element TextColor 설정
*	@param	HDC hDC			:	disable text color
*	@param	RECT &rcUpdate	:	enable text color
*	@retval	BOOL FALSE : 실패, TRUE : 성공
*****************************************************************/
BOOL CNTableEditTextBox::DrawControl(HDC hDC, RECT &rcUpdate)
{
	...
	for (row = 1; row < MAX_TABLE_ROW && row-1 <= m_nAllowRow; row++)
	{
		for (col = 40; col < MAX_TABLE_COL; col++)
		{
			...
			{
				HFONT		hOldFont;
				COLORREF	nOldTextColor = 0xffffffff;
				HBRUSH		hOldBrush;
...
/** *************************************************************
*	@brief	Key Event 발생시 수행
*	@param	CString &strKey	:	입력받은 key
*	@retval	BOOL FALSE : 실패, TRUE : 성공
*****************************************************************/
			...
			if (strKey.GetLength() == 1)
			{
				...
				//	if( (_T("9") == strKey) && (SPECIALCHAR == g_nCurrEditMode) )
					if( ((_T("9") == strKey) && (SPECIALCHAR == g_nCurrEditMode)) || ((_T("0") == strKey) && (NUMBER != g_nCurrEditMode)) )
						m_bCharInputComplete = FALSE;	// 특수문자 '9'(blank) 이후에도 모드 변경시, 커서가 움직이면 안됨.
					else
...
/** *************************************************************
*	@brief	AP로부터 TableEditBox에 설정
*	@param	CString &strName :	control name
*	@param	CString &strData :	설정할 data
*	@retval	BOOL FALSE : 실패, TRUE : 성공
*****************************************************************/
BOOL CNTableEditTextBox::SetVariable(CString &strName, CString &strData)
{
	if (m_strName == strName)
	{
		...
		{
			// Split String.
			CUtil::ParsingStringToStringArray(strData, _T("\n"), strTempArray);
...
/** *************************************************************
*	@brief	모든 TableEditBox의 문자들을 하나의 문자열로 저장
*	@param	CString &strData :	문자열 data
*	@retval	BOOL FALSE
*****************************************************************/
	...
	else if ( (strOrgKey.CompareNoCase(_T("0")) >= 0) && (strOrgKey.CompareNoCase(_T("9")) <= 0) )
	{
				  ...
				  (SPECIALCHAR == g_nCurrEditMode) )	// @ADD: LEH - 특수문자 추가
		{
			...
			if (UPPERCASE == g_nCurrEditMode)
			{
				...
				mapPinChar.Add(_T("QZ"));	// 1
				mapPinChar.Add(_T("ABC"));	// 2
				mapPinChar.Add(_T("DEF"));	// 3
				mapPinChar.Add(_T("GHI"));	// 4
				mapPinChar.Add(_T("JKL"));	// 5
				mapPinChar.Add(_T("MNO"));	// 6
				mapPinChar.Add(_T("PRS"));	// 7
				mapPinChar.Add(_T("TUV"));	// 8
				mapPinChar.Add(_T("WXY"));	// 9
			}
...
//	if( (NUMBER == g_nCurrEditMode) && (_T("0") != strKey) )
	if(NUMBER == g_nCurrEditMode)
	{
		...
		// 숫자모드에서는 "0"을 누른 후에 다른 숫자를 누를 경우 커서가 한칸 더 이동해야 함.
...
BOOL CNTableEditTextBox::SetInputMode(CString &strInputMode)
{
	...
	else if(strInputMode.CompareNoCase(_T("HEXADECIMAL")) == 0)
		g_nCurrEditMode = HEXADECIMAL;
	else
		g_nCurrEditMode = NUMBER;
...
BOOL CNButtonBox::MouseEvent(POINT &point)
{
	if (m_bShow)
	{
		if (PtInRect(&m_rcDisplay, point))
		{
			// Touch or Mouse일 경우 Sound 소리나도록 추가함
...
BOOL CNButtonBox::KeyEvent(CString &strKey)
{
	if (m_bShow)
	{
		// 1자는 [1, L1 or R1]을 Find에서 같게 처리함으로 제외함.
...
CNChangePWBox::CNChangePWBox(CTL_TYPE type) : CNEditTextBox(type)
{
	// Password type.
...
BOOL CNChangePWBox::KeyEvent(CString &strKey)
{
	if (m_bFocus)
	{
		...
		if (strKey == DES_ENTER)
		{
			...
			{
				// Edit Control 하위 클래스 최소 입력 글자 설정 기능 추가
				...
				else
				{
					...
					else
					{
						...
						// Password Verification 실패시 New Password부터 다시 입력
...
{
	if (strKey == DES_STAR || strKey == DES_SHARP)				//'.' 입력 선택 적용.
	{
		if (strKey == DES_STAR)									// '.' 입력 선택 적용.
			m_nSwKeyDataIndex--;
		else 
			m_nSwKeyDataIndex++;
	...
	else if (strKey == DES_CAPS)							// '.' 입력 선택 적용.
	{
...
BOOL CNVirtualKeyBox::DrawControl(HDC hDC, RECT &rcUpdate)
{
	...
	for (row = 0; row < MAX_KEY_ROW; row++)
	{
		for (col = 0; col < MAX_KEY_COL; col++)
		{
			...
			{
				int			OldBkMode;
				HFONT		OldKeyFont = NULL;
				COLORREF	OldKeyColor = 0xffffffff;
...
/** **********************************************************
*	@brief	On-screen keyboard key label built from an ASCII code.
*
*	The tables below were written as (LPCTSTR)0xNN, i.e. a POINTER holding the
*	ASCII code. CString treats any value under 0x10000 as a string RESOURCE ID
*	and calls LoadString(), which requires an initialised MFC. Since these are
*	file-scope globals they run before WinMain, so on Win32 that trips
*	ASSERT(afxCurrentResourceHandle != NULL) (afxwin1.inl line 24).
*
*	WinCE keeps the original expression untouched; Win32 builds the intended
*	single-character string.
************************************************************/
#ifdef _WIN32_WCE
#define KEYCHR(c)	((LPCTSTR)(c))
#else
#define KEYCHR(c)	CString((TCHAR)(c), 1)
#endif

CString	strUpperKeys[5][10] = 
{
	// row 0
	{ KEYCHR(0x30), KEYCHR(0x31), KEYCHR(0x32), KEYCHR(0x33), KEYCHR(0x34), KEYCHR(0x35), KEYCHR(0x36), KEYCHR(0x37), KEYCHR(0x38), KEYCHR(0x39) },
	// row 1
...
CString	strNumberKeys[5][10] = 
{
	...
	// row 3
	{ _T(")"), _T("]"), _T("}"), _T(">"), _T("."), _T("$"), KEYCHR(0x22), _T("*"), _T(";"), _T("/") },
	// row 4
	{_T("CHARACTER SELECT : 0 ~ 9  SELECT : ENTER")}
};
...
void CNAlphaKeyBox::ChangeInputMode()
{
	...
	// 문자입력 후, Mode Change로 입력모드를 한 바퀴 로테이션 돈 다음 문자를 입력하게 되면 overwrite 되기 때문에 주석 처리함.
	// 또는 특수문자 입력 후, 대문자(or 소문자) 입력시 overwrite 됨.
	// ex) 'A'입력 후, Mode Change 4번 후, 'D'를 입력하게 되면 "AD"가 되는 것이 아니라 "D"가 됨.
	//     특수 문자 '='입력 후, 'A'입력 시 "=A" 가 아닌 "A"가 됨. 
// 	if(NUMBER == g_nCurrEditMode)
// 		g_bIsPrevNotNumberMode = FALSE;
...
{ 
	// row 0
	{ KEYCHR(0x20), KEYCHR(0x21), KEYCHR(0x22), KEYCHR(0x23), KEYCHR(0x24), KEYCHR(0x25),    _T("&&"), KEYCHR(0x27), KEYCHR(0x28), KEYCHR(0x29),
	  KEYCHR(0x2A), KEYCHR(0x2B), KEYCHR(0x2C), KEYCHR(0x2D), KEYCHR(0x2E), KEYCHR(0x2F), KEYCHR(0x30), KEYCHR(0x31), KEYCHR(0x32) },
	// row 1
	{ KEYCHR(0x33), KEYCHR(0x34), KEYCHR(0x35), KEYCHR(0x36), KEYCHR(0x37), KEYCHR(0x38), KEYCHR(0x39), KEYCHR(0x3A), KEYCHR(0x3B), KEYCHR(0x3C),
	  KEYCHR(0x3D), KEYCHR(0x3E), KEYCHR(0x3F), KEYCHR(0x40), KEYCHR(0x41), KEYCHR(0x42), KEYCHR(0x43), KEYCHR(0x44), KEYCHR(0x45) },
	// row 2
	{ KEYCHR(0x46), KEYCHR(0x47), KEYCHR(0x48), KEYCHR(0x49), KEYCHR(0x4A), KEYCHR(0x4B), KEYCHR(0x4C), KEYCHR(0x4D), KEYCHR(0x4E), KEYCHR(0x4F),
	  KEYCHR(0x50), KEYCHR(0x51), KEYCHR(0x52), KEYCHR(0x53), KEYCHR(0x54), KEYCHR(0x55), KEYCHR(0x56), KEYCHR(0x57), KEYCHR(0x58) },
	// row 3
	{ KEYCHR(0x59), KEYCHR(0x5A), KEYCHR(0x5B), KEYCHR(0x5C), KEYCHR(0x5D), KEYCHR(0x5E), KEYCHR(0x5F), KEYCHR(0x60), KEYCHR(0x61), KEYCHR(0x62),
	  KEYCHR(0x63), KEYCHR(0x64), KEYCHR(0x65), KEYCHR(0x66), KEYCHR(0x67), KEYCHR(0x68), KEYCHR(0x69), KEYCHR(0x6A), KEYCHR(0x6B) },
	// row 4
	{ KEYCHR(0x6C), KEYCHR(0x6D), KEYCHR(0x6E), KEYCHR(0x6F), KEYCHR(0x70), KEYCHR(0x71), KEYCHR(0x72), KEYCHR(0x73), KEYCHR(0x74), KEYCHR(0x75),
	  KEYCHR(0x76), KEYCHR(0x77), KEYCHR(0x78), KEYCHR(0x79), KEYCHR(0x7A), KEYCHR(0x7B), KEYCHR(0x7C), KEYCHR(0x7D), KEYCHR(0x7E) },
	// row 5
...
CString CNCentBox::GetDrawText()
{
	...
	// 00321 -> 321
	// 320 -> 320
	if (CUtil::StringToInt(m_strText) == 0)
	{
		m_strText = _T("");
	}
	...
	if (m_strText.GetLength() <= 2)
	{
		// 0 -> 0.00
		// 10 -> 0.10
		RightValue = CUtil::StringToInt(m_strText.Right(2));
...
CString CNDollarBox::GetDrawText()
{
	...
	strDrawText.Format(_T("%s %d"), CURRENCY_SYMBOL, CUtil::StringToInt(m_strText));
	
	return strDrawText;
}

// "CLEAR" => Remove all entered amount
BOOL CNDollarBox::KeyEvent(CString &strKey)
{
	if (m_bFocus)
	{
		...
		// numeric key.
				 ...
				 (m_TextType == TTYPE_IPADDRESS && strKey == DES_STAR))
		{
			if (m_TextType == TTYPE_IPADDRESS && strKey == DES_STAR)
				strKey = _T(".");									
...
BOOL CNAnimation::SetAniImage(int Count, CString strPrefix)
{
	...
	for (int i=0; i < Count; i++)
	{
		...
		//strFileName[i].Format(_T("%s%d.png"),strPrefix, i+1);
...
//	m_Image.DecodeImage(0);
	m_Image.DecodeImage4Adv(0);	// V1.0.2.5 2018.08.06 - Adv를 위한 decode
...
CString CNInputEditBox::GetDrawText()
{
	...
	if(m_nInputType != 0)
	{
		if(0 == CUtil::StringToInt(m_strText))
			m_strText.Empty();
	}
	...
	else if(3 == m_nInputType)	// percent
	{
		if (m_strText.GetLength() <= 2)
		{
			// 0 -> 0.00
...
{
	m_nInputType = CUtil::StringToInt(strData);
...
BOOL CNThreeTypeBox::SetVariable(CString &strName, CString &strData)
{
	if (m_strNameEx[0] == strName)
	{
		...
		// Disable
		if (_tcsncmp(strData, DES_DISABLE, _tcslen(DES_DISABLE)) == 0)
		{
			m_nCurState = STATE_DISABLE;
		}
		// Enable
		else if (_tcsncmp(strData, DES_ENABLE, _tcslen(DES_ENABLE)) == 0)
		{
			m_nCurState = STATE_ENABLE;
		}
		// Select
...
*	@retval	없음
*****************************************************************/
BOOL CNEditButtonBox::MakeAllFSCMD(CString &Data)
{
	Data += SCR_RES_DELIMITER + m_strText;
