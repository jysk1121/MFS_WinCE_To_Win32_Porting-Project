...
template <class T> 
CPtrLinkedList<T>::CPtrLinkedList() : m_cNotEmpty(TRUE, FALSE), m_cCritSec()
{
...
template <class T> 
CPtrLinkedList<T>::~CPtrLinkedList()
{
	Cleanup();
}

template <class T> 
CPtrLinkedList<T>& CPtrLinkedList<T>::operator= (const CPtrLinkedList<T> &rList)
{
	T	*pData;
...
template <class T> 
BOOL CPtrLinkedList<T>::PutOnHeadOfList( CString Key, T *pData )
{
...
template <class T> 
BOOL CPtrLinkedList<T>::PutOnTailOfList( CString Key, T *pData )
{
	CListNode *pNewNode = NULL;
...
template <class T> 
BOOL CPtrLinkedList<T>::GetFromHeadOfList( T **pData, DWORD dwTimeout )
{
	CListNode	*pNode = NULL;
...
template <class T> 
BOOL CPtrLinkedList<T>::GetFromTailOfList( T **pData, DWORD dwTimeout )
{
	CListNode	*pNode = NULL;
	CListNode	*pPrevNode = NULL;
	CListNode	*pNextNode = NULL;
...
template <class T> 
int CPtrLinkedList<T>::GetDataCount()
{
	return m_nDataCount;
}

template <class T> 
BOOL CPtrLinkedList<T>::FindDataOfList(CString Key, T **pData, BOOL bRemove)
{
	CListNode	*pNode = NULL;
...
template <class T> 
BOOL CPtrLinkedList<T>::FindDataOneOfList(CString Key, T **pData)
{
	CString		strNodeKey;
	CListNode	*pNode = NULL;
...
template <class T> 
BOOL CPtrLinkedList<T>::MoveFirst(T **pData)
{
	// 리스트에 원소가 생길 때까지 대기한다.
	if (m_cNotEmpty.Wait(0) != WAIT_OBJECT_0)
		return FALSE;
...
template <class T> 
BOOL CPtrLinkedList<T>::MoveLast(T **pData)
{
	// 리스트에 원소가 생길 때까지 대기한다.
	if (m_cNotEmpty.Wait(0) != WAIT_OBJECT_0)
		return FALSE;
...
template <class T> 
BOOL CPtrLinkedList<T>::MovePrev(T **pData, int nSize)
{
	// 리스트에 원소가 생길 때까지 대기한다.
	if (m_cNotEmpty.Wait(0) != WAIT_OBJECT_0)
		return FALSE;
...
template <class T> 
BOOL CPtrLinkedList<T>::MoveNext(T **pData, int nSize)
{
	// 리스트에 원소가 생길 때까지 대기한다.
	if (m_cNotEmpty.Wait(0) != WAIT_OBJECT_0)
		return FALSE;
...
template <class T> 
void CPtrLinkedList<T>::Sort(BOOL bAsc)
{
	// 리스트에 원소가 생길 때까지 대기한다.
	if (m_cNotEmpty.Wait(0) != WAIT_OBJECT_0)
		return;
...
template <class T> 
void CPtrLinkedList<T>::DeleteAll( void )
{
	CListNode	*pNode = NULL;
	CListNode	*pPrevNode = NULL;
	CListNode	*pNextNode = NULL;
...
template <class T> 
void CPtrLinkedList<T>::CleanFreeNode( void )
{
	//리스트의 임계 영역 락을 획득한다.
	CAutoLock autoLock( m_cCritSec );
...
template <class T> 
void CPtrLinkedList<T>::Cleanup( void )
{
	// Master List 삭제.
	DeleteAll();
...
template <class T> 
BOOL CPtrLinkedList<T>::IsHasDataOfList( DWORD dwTimeout )
{
	// 리스트에 원소가 생길 때까지 대기한다.
	if (m_cNotEmpty.Wait(dwTimeout) != WAIT_OBJECT_0)
		return FALSE;
...
template <class T> 
void CPtrLinkedList<T>::AddToFreeList( CListNode* pNewNode )
{
...
template <class T> 
void CPtrLinkedList<T>::InsertBetween(CListNode *pPrevNode, CListNode *pNextNode, CListNode *pNewNode)
{
