...
typedef enum REQ_TYPE
{
	...
	REQ_LOCALE,
	REQ_SETVALUE,
	REQ_MOUSEEVENT,		// [2ND] NH AIREAT 2008.11.12
	REQ_KEYEVENT,
	REQ_SETFOCUS,
	REQ_BTNFIND,
	REQ_FSCMD,
	...
	REQ_BINACT,
	REQ_BLINKING,
	REQ_FULLFSCMD,		// [#75] NH KGS 2008.04.02 Full FS Command 추가
	REQ_MAX,
	REQ_SETINPUTMODE,
	REQ_KEYDISABLE,
	REQ_SETINPUTTYPE,
...
public:
	CRequest();
	CRequest(REQ_TYPE type, CString Name, CString Data);//CRequest(REQ_TYPE type, LPCTSTR Name, LPCTSTR Data);
	CRequest(const CRequest &rRequest);
public:
	void		Set(REQ_TYPE type, LPCTSTR Name, LPCTSTR Data);
	REQ_TYPE	GetKind();
	CString		GetName();//LPCTSTR		GetName();
	CString		GetData();//LPCTSTR		GetData();
	void		SetKinD(REQ_TYPE type);
	void		SetName(CString Name);//void		SetName(LPCTSTR Name);
	void		SetData(CString Data);//void		SetData(LPCTSTR Data);
...
public:
	...
	POINT		m_Point;		// for Mouse point		// [2ND] NH AIREAT 2008.11.12
};
...
typedef enum CTL_TYPE
{
	...
	CTL_ANI,
	CTL_ADVBOX,
	CTL_BININDEXBOX,		//[#543]SOOK 2009.07.17 BIN INDEX BOX 추가 
	CTL_DECIMALBOX,			//[#774]Justin 2014.12.16 Add DecimalBox
	CTL_CENTCLEARBOX,		//[#810]Justin 2015.10.09 Cent Clear Box
	CTL_MAX,
	CTL_KEYTESTBOX,
	CTL_INPUTEDITBOX,
	CTL_THREETYPEBOX,
...
private:
	...
	RECT		GetAdjustScreenReate(int left, int top, int width, int height);

private:
	static float		m_fScreenRateX;		// current screen X에 대한 640의 비율 
...
typedef enum TEXTTYPE
{
	TTYPE_TEXT=0,
	TTYPE_PASSWORD,
	TTYPE_IPADDRESS					// [#470] NH AIREAT 2008.12.22 : IP ADDRESS type 추가.
};
...
//	void	SetTextColor(COLORREF NormalColor, COLORREF FocusColor);
...
//	COLORREF		m_crTextColor, m_crFocusTextColor;
	TEXTALIGN		m_Align;
	BOOL			m_bMultiline;
	CRequest		m_Act;
	CString			m_ActKey;
...
//	CNFont			*m_pGuideFont;
//	CNFont			*m_pFont, *m_pFocusFont;
//	CNBrush			*m_pNormalElementBrush, *m_pFocusElementBrush;
//	CNColor			*m_pNormalTextColor, *m_pFocusTextColor;
};
...
class CNChangePWBox : public CNEditTextBox	// [#16] NH KGS 2008.03.17 CNTextBox -> CNEditTextBox
{
public:
...
private:
	...
	int			m_nLoadImage;
	int			m_nBlinkCount;
//	CString		m_strImageName[MAX_ANI];
...
// [#810] US Justin 2015.10.16 Add CentClearBox. Clearing entire amount
...
*	@brief		Dollar, Cent, Percent를 보여주기 위한 클래스
*****************************************************************/
