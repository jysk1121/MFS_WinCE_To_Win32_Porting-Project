...
void CNScreen::DeleteControl()
{
	...
	while (m_Controls.GetFromHeadOfList(&pControl, 0))
	{
		...
		else if (pControl->GetType() == CTL_BININDEXBOX) //[#543] SOOK 2007.07.19
		{
			CNBinIndexBox  *g_pNBinIndexBox = (CNBinIndexBox*)pControl;
			delete g_pNBinIndexBox;
		}
		else if (pControl->GetType() == CTL_DECIMALBOX)	//[#774] Justin 2014.12.16 Add Decimal Box
		{
			CNDecimalBox  *g_pNDecimalBox = (CNDecimalBox*)pControl;
			delete g_pNDecimalBox;
		}
		else if (pControl->GetType() == CTL_CENTCLEARBOX)	//[#810] Justin 2015.10.09 Add CentClear Box
		{
			CNCentClearBox  *g_pNCentClearBox = (CNCentClearBox*)pControl;
			delete g_pNCentClearBox;
		}
		// V1.0.2.3 2018.06.26 - GIFBOX 추가
...
void CNScreen::Initialize()
{
	...
	m_nScreenTimeout = 0;			// [#236] NZ AIREAT 2008.05.27
...
//		if ((Req.GetKind() == REQ_FSCMD) || (Req.GetKind() == REQ_ALLFSCMD))
...
BOOL CNScreen::GetFSCmd(CRequest &FSCmd)
{
	if ((m_FSCMD.GetKind() == REQ_FSCMD) ||
		(m_FSCMD.GetKind() == REQ_FULLFSCMD) ||		// [#75] NH KGS 2008.04.02 Full FS Command 추가
